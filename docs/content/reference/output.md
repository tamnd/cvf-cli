...
describes what users get. Pick a format with `-o`, or let cvf choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
cvf <command> -o table   # aligned columns for reading
cvf <command> -o jsonl   # one JSON object per line, for piping
cvf <command> -o json    # a single JSON array
cvf <command> -o csv     # spreadsheet friendly
cvf <command> -o tsv     # tab-separated
cvf <command> -o url     # just the URL column
cvf <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
cvf <command> --fields id,title,url
```
...
```bash
cvf <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
cvf <command>            # a table, because this is a terminal
cvf <command> | wc -l    # JSONL, because this is a pipe
```
